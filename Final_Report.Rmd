...
```{r figure-1-Enrichment-Plot, echo = FALSE}
...
joined_table %>%
  ...
  geom_col() +
  coord_flip() +
  scale_fill_brewer(palette = "Spectral", name = "Gene ") +
...
```{r figure-2-PCA-Plot, echo = FALSE}
...
```{r figure-3-heatmap-inhibitory-cytokines-and-ligands, echo = FALSE}
inverse_func <- function(x) {
  if (x >= 1) {
    return(x)
  } else {
...
joined_table %>%
  ...
  scale_fill_viridis_c(name = "Fold Change \nExpression") +
...
```{r figure-4-heatmap-inhibitory-receptors, echo = FALSE}
inverse_func <- function(x) {
  if (x >= 1) {
    return(x)
  } else {
...
```{r figure-5-heatmap-chemokine-receptors, echo = FALSE}
inverse_func <- function(x) {
  if (x >= 1) {
    return(x)
  } else {
...
joined_table %>%
  ...
  xlab("Psoriasis") +
  ylab("Gene") +
  ggtitle("Fold Change Expression of Cell Adhesion \nGenes in Psoriasis Types") +
  scale_fill_viridis_c(name = "Fold Change \nExpression") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_x_discrete(labels=c("fold_expression_p" = "Conventional \nPsoriasis",
                            "fold_expression_pp" = "Palmoplantar \nPsoriasis",
...
install.packages("gplots")# Inhibitory mechanisms
joined_table %>%
